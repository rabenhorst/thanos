...
func expandChunkIterator(it chunkenc.Iterator, encoding chunkenc.Encoding, samples *[]sample) error {
	...
	default:
		return errors.Errorf("unexpected chunk encoding %s", encoding)
	}
}
...
func (f *floatAggregator) add(s sample) {
	if f.total > 0 {
		if s.v < f.last {
...
type histogramAggregator struct {
	...
	resetDetected bool                      // Whether a reset was detected since the beginning.
}
...
func DownsampleRaw(data []sample, resolution int64) []chunks.Meta {
	...
	numChunks := targetChunkCount(min, max, 1*60*1000, resolution, len(data))
...
func downsampleRawLoop(
	...
	numChunks int,
	chks *[]chunks.Meta,
	downsampleBatch func(batch []sample, resolution int64) chunks.Meta,
) {
	if len(data) == 0 {
		return
	}
	...
	for len(data) > 0 {
		...
		*chks = append(*chks, downsampleBatch(batch, resolution))
	}
}
